...
export const FacultyDashboard: React.FC<FacultyDashboardProps> = ({ user, searchTerm: globalSearchTerm, filterStatus, hideImplemented }) => {
  ...
  // Form state
  const [type, setType] = useState<'marks' | 'date' | 'venue' | 'revaluation'>('marks');
    ...
    try {
      const res = await fetch('/api/requests', {
        ...
        body: JSON.stringify({
          ...
          type,
          studentName,
          studentRegNum: studentRegNum.trim(),
          subjectName,
          description,
          oldValue,
          newValue
      ...
      console.error(err);
      setSubmitError(err instanceof Error ? err.message : 'An error occurred while submitting the request');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        ...
        <div className="bg-white p-6 rounded-2xl border border-zinc-200 shadow-sm">
          <p className="text-sm font-medium text-zinc-400">Expired</p>
          <h4 className="text-3xl font-bold text-zinc-800 mt-1">{stats.expired}</h4>
        </div>
      </div>

      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-zinc-800">My Change Requests</h2>
          <p className="text-zinc-500">Track and manage your academic change submissions</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button 
            ...
            className="flex items-center gap-2 bg-brand-600 hover:bg-brand-700 text-white px-5 py-2.5 rounded-xl transition-all shadow-md hover:shadow-lg active:scale-95"
          >
            <PlusCircle size={20} />
            New Request
          </button>
      ...
      <AnimatePresence>
        {isFormOpen && (
          <motion.div 
            ...
            className="bg-white p-8 rounded-2xl border border-brand-100 shadow-xl relative overflow-hidden"
          >
            ...
            <h3 className="text-lg font-bold text-zinc-800 mb-6 flex items-center gap-2">
              ...
              Submit New Academic Change Request
            </h3>
            <ErrorAlert message={submitError} onClose={() => setSubmitError(null)} />
            <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-zinc-700">Student Name</label>
                    <input 
                      type="text"
                      value={studentName}
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-zinc-700">Registration Number</label>
                    <input 
                      type="text"
                      value={studentRegNum}
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-zinc-700">Subject Name</label>
                    <input 
                      type="text"
                      value={subjectName}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="text-sm font-bold text-zinc-700">Request Category</label>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                    {(['marks', 'date', 'venue', 'revaluation'] as const).map((t) => (
                      <button
                        key={t}
                        type="button"
                        onClick={() => {
                      ...
                      >
                        {getTypeIcon(t)}
                        <span className="capitalize text-[10px] md:text-sm">
                          {t === 'date' ? 'Exam Date' : t === 'venue' ? 'Exam Venue' : t === 'marks' ? 'Internal Marks' : t === 'revaluation' ? 'Revaluation' : t}
                        </span>
                      </button>
                    ))}
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-zinc-700">
                      {type === 'marks' ? 'Old Internal Mark' : type === 'date' ? 'Old Exam Date' : type === 'venue' ? 'Old Exam Venue' : 'Old Result'}
                    </label>
                    {type === 'revaluation' ? (
                      <select
                        value={oldValue}
                        ...
                        required
                        className="w-full p-3 rounded-xl border border-zinc-200 focus:ring-2 focus:ring-brand-500 outline-none bg-white"
                      >
                      ...
                      <input 
                        ...
                        onChange={(e) => setOldValue(e.target.value)}
                        required
                        placeholder={type === 'marks' ? 'e.g., 45' : type === 'venue' ? 'e.g., Hall A' : ''}
                        className="w-full p-3 rounded-xl border border-zinc-200 focus:ring-2 focus:ring-brand-500 outline-none"
                      />
                    )}
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-zinc-700">
                      {type === 'marks' ? 'New Internal Mark' : type === 'date' ? 'New Exam Date' : type === 'venue' ? 'New Exam Venue' : 'New Result'}
                    </label>
                    {type === 'revaluation' ? (
                      <select
                        value={newValue}
                      ...
                      >
                        ...
                        <option value="Pass (Grade O)">Pass (Grade O)</option>
                        <option value="Pass (Grade A+)">Pass (Grade A+)</option>
                        <option value="Pass (Grade A)">Pass (Grade A)</option>
                        <option value="Pass (Grade B+)">Pass (Grade B+)</option>
                        <option value="Pass (Grade B)">Pass (Grade B)</option>
                        <option value="Pass (Grade C)">Pass (Grade C)</option>
                      ...
                      <input 
                        ...
                        onChange={(e) => setNewValue(e.target.value)}
                        required
                        placeholder={type === 'marks' ? 'e.g., 48' : type === 'venue' ? 'e.g., Hall B' : ''}
                        className="w-full p-3 rounded-xl border border-zinc-200 focus:ring-2 focus:ring-brand-500 outline-none"
                      />
                    )}
                  </div>
              ...
              <div className="space-y-2">
                <label className="text-sm font-bold text-zinc-700">Detailed Description & Justification</label>
                <textarea 
                  value={description}
                  onChange={(e) => setDescription(e.target.value)}
                  required
                  rows={4}
                  placeholder="Provide clear details about the change and why it is necessary..."
                  className="w-full p-4 rounded-xl border border-zinc-200 focus:ring-2 focus:ring-brand-500 focus:border-brand-500 outline-none transition-all"
                />
              </div>
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button 
                  type="submit"
                  className="flex items-center gap-2 bg-brand-600 hover:bg-brand-700 text-white px-8 py-2.5 rounded-xl transition-all font-bold shadow-md"
                >
                  Submit to HOD
                </button>
              </div>
      ...
      <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-zinc-100 flex flex-col md:flex-row justify-between gap-4 bg-zinc-50/30">
          ...
          <div className="relative">
            ...
            >
              <Filter size={18} />
              {filterType === 'all' ? 'Filter' : filterType === 'marks' ? 'Internal Marks' : filterType === 'date' ? 'Exam Date' : 'Exam Venue'}
            </button>
        ...
        <div className="divide-y divide-zinc-100">
          {loading ? (
            ...
            <div className="p-12 text-center">
              ...
              <p className="text-zinc-500 mt-1">Try adjusting your search or create a new request.</p>
            </div>
          ) : (
            filteredRequests.map((req) => (
              <motion.div 
                ...
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="p-6 hover:bg-zinc-50/80 transition-all group"
              >
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                  <div className="flex items-start gap-4 flex-1">
                    ...
                    <div className="space-y-1">
                      ...
                      {(req.student_name || req.student_reg_num || req.subject_name) && (
                        <div className="flex items-center gap-2 text-sm font-bold text-zinc-700">
                          {req.student_name && <span>{req.student_name}</span>}
                          {req.student_name && req.student_reg_num && <span className="text-zinc-300">|</span>}
                          {req.student_reg_num && <span className="text-zinc-500">{req.student_reg_num}</span>}
                          {(req.student_name || req.student_reg_num) && req.subject_name && <span className="text-zinc-300">|</span>}
                          {req.subject_name && <span className="text-brand-600">{req.subject_name}</span>}
                        </div>
                      )}
                  ...
                  <div className="flex flex-col items-end gap-4 min-w-[200px]">
                    ...
                    {(req.hod_comment || req.academic_comment) && (
                      <div className="bg-zinc-50 p-3 rounded-xl border border-zinc-100 w-full">
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-zinc-900/60 backdrop-blur-sm">
            ...
            >
              ...
              <div className="p-8 space-y-8 max-h-[70vh] overflow-y-auto">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="space-y-1">
                    <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">Student Information</p>
                    <p className="text-sm font-bold text-zinc-700">{selectedRequest.student_name}</p>
                    <p className="text-xs text-zinc-500">Reg No: {selectedRequest.student_reg_num}</p>
                  </div>
                  <div className="space-y-1">
